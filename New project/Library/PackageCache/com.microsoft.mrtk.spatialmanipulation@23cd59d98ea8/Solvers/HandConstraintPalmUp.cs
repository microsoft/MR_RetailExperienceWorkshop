    ...
    public class HandConstraintPalmUp : HandConstraint
    {
        ...
        private bool IsUserGazeMeetingThresholdRequirements(XRNode hand)
        {
            using (IsUserGazeMeetingThresholdRequirementsPerfMarker.Auto())
            {
                ...
                {
                    XRControllerState controllerState = controllerLookup.GazeController.currentControllerState;
                    if ((controllerState != null) &&
                        ((controllerState.inputTrackingState & (InputTrackingState.Position | InputTrackingState.Rotation)) > 0))
                    {
                        gazeRay = new Ray(
