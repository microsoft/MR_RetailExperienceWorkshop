...
namespace Microsoft.MixedReality.Toolkit.UX
{
    ...
    public class ObjectBar : MonoBehaviour
    {
        ...
        public void UpdateObjectBar()
        {
            ...
            startCalculationPosition = transform.position;
        ...
        private void CalculateObjectPositions()
        {
            ...
            {
                ObjectBarObjects[0].transform.position = startCalculationPosition;

                for (int i = 1; i < ObjectBarObjects.Count; i++)
                {
                    Vector3 newPosition;
                    Vector3 previousObjectPosition = ObjectBarObjects[i - 1].transform.position;

                    if (ObjectBarFlowDirection == ObjectBarFlowDirection.Horizontal)
                    {
                        float averageXDistance = (colliders[i - 1].bounds.size.x + colliders[i].bounds.size.x) * 0.5f;
                        newPosition = previousObjectPosition + (averageXDistance * Vector3.right) + (Vector3.right * SpacingBetween.x);
                    }
                    else
                    {
                        float averageYDistance = (colliders[i - 1].bounds.size.y + colliders[i].bounds.size.y) * 0.5f;
                        newPosition = previousObjectPosition + (averageYDistance * Vector3.up) + Vector3.up * SpacingBetween.y;
                    }

                    ObjectBarObjects[i].transform.position = newPosition;
                }
            }
        }
        ...
        private void CalculateBackPlateSize()
        {
            if (BackPlateObject != null && ObjectBarObjects.Count != 0 && ObjectBarObjects[0] != null)
            {
                ...
                float distanceFirstToLast = Vector3.Distance(ObjectBarObjects[0].transform.position, ObjectBarObjects[ObjectBarObjects.Count - 1].transform.position);
                ...
                if (ObjectBarFlowDirection == ObjectBarFlowDirection.Horizontal)
                {
                    ...
                    newPosition = transform.position + (Vector3.right * (distanceFirstToLast * 0.5f - largestColliderSizes.x * 0.5f)) + Vector3.back * BackPlateZOffset;
                }
                else // ObjectBarFlowDirection.Vertical
                {
                    ...
                    newPosition = transform.position + ((Vector3.up) * (distanceFirstToLast * 0.5f - largestColliderSizes.y * 0.5f)) + Vector3.back * BackPlateZOffset;
                }

                BackPlateObject.transform.localScale = newScale;
                BackPlateObject.transform.position = newPosition;
            }
        }
        ...
        private Vector2 GetLargestColliderSizes()
        {
            ...
            {
                if (colliders[i].bounds.size.x > largestXSize)
                {
                    largestXSize = colliders[i].bounds.size.x;
                }

                if (colliders[i].bounds.size.y > largestYSize)
                {
                    largestYSize = colliders[i].bounds.size.y;
                }
            }
