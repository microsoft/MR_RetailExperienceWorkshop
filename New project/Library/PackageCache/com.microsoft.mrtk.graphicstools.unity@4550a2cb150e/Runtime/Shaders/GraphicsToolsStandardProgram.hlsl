...
#if defined(_ALPHATEST_ON) || defined(UNITY_UI_CLIP_RECT) || defined(UNITY_UI_ALPHACLIP) || defined(_CLIPPING_PRIMITIVE) || defined(_ROUND_CORNERS)
#define _ALPHA_CLIP
#else
#undef _ALPHA_CLIP
#endif
...
Varyings VertexStage(Attributes input)
{
    ...
    for (int hoverLightIndex = 0; hoverLightIndex < HOVER_LIGHT_COUNT; ++hoverLightIndex)
    {
        ...
        fadeDistance = min(fadeDistance, NearLightDistance(_HoverLightData[dataIndex], output.worldPosition));
    }
    ...
    for (int proximityLightIndex = 0; proximityLightIndex < PROXIMITY_LIGHT_COUNT; ++proximityLightIndex)
    {
        ...
        fadeDistance = min(fadeDistance, NearLightDistance(_ProximityLightData[dataIndex], output.worldPosition));
    }
#else
    ...
    // Flip the y coordinate of the screen position when rendered with an orthographic camera.
...
#if defined(_URP)
    output.gradient = Iridescence(tangentDotIncident, 
...
// Texturing.
    ...
    primitiveDistance = min(primitiveDistance, PointVsPlane(input.worldPosition.xyz, _ClipPlane) * _ClipPlaneSide);
#endif
#if defined(_CLIPPING_SPHERE)
    primitiveDistance = min(primitiveDistance, PointVsSphere(input.worldPosition.xyz, _ClipSphereInverseTransform) * _ClipSphereSide);
#endif
#if defined(_CLIPPING_BOX)
    primitiveDistance = min(primitiveDistance, PointVsBox(input.worldPosition.xyz, _ClipBoxInverseTransform) * _ClipBoxSide);
#endif
...
#endif
#if defined(_USE_WORLD_SCALE)
    float cornerCircleRadius = max(currentCornerRadius, _MinCorverValueWorld) * input.scale.z;
#else
    float cornerCircleRadius = saturate(max(currentCornerRadius - _RoundCornerMargin, _MinCorverValue)) * input.scale.z;
#endif
    float2 cornerCircleDistance = halfScale - (_RoundCornerMargin * input.scale.z) - cornerCircleRadius;
#if defined(_ROUND_CORNERS)
    float roundCornerClip = RoundCorners(cornerPosition, cornerCircleDistance, cornerCircleRadius, _EdgeSmoothingValue * input.scale.z);
...
#if defined(_IRIDESCENCE)
    half3 gradientColor = input.gradient;
#elif defined(_GRADIENT_FOUR_POINT)
    half3 gradientColor = FourPointGradient(_GradientColor0, _GradientColor1, _GradientColor2, _GradientColor3, _GradientColor4, input.uv);
#elif defined(_GRADIENT_LINEAR)
    half3 gradientColor = LinearGradient(_GradientColor0, _GradientColor1, _GradientColor2, _GradientColor3, input.gradient);
#endif
    ...
    {
        int dataIndex = hoverLightIndex * HOVER_LIGHT_DATA_SIZE;
        half hoverValue = HoverLight(_HoverLightData[dataIndex], _HoverLightData[dataIndex + 1].w, input.worldPosition.xyz);
    ...
    for (int proximityLightIndex = 0; proximityLightIndex < PROXIMITY_LIGHT_COUNT; ++proximityLightIndex)
    {
        int dataIndex = proximityLightIndex * PROXIMITY_LIGHT_DATA_SIZE;
        half colorValue;
        half proximityValue = ProximityLight(_ProximityLightData[dataIndex], _ProximityLightData[dataIndex + 1], _ProximityLightData[dataIndex + 2], input.worldPosition.xyz, worldNormal, colorValue);
        pointToLight += proximityValue;
#if defined(_PROXIMITY_LIGHT_COLOR_OVERRIDE)
        half3 proximityColor = MixProximityLightColor(_ProximityLightCenterColorOverride, _ProximityLightMiddleColorOverride, _ProximityLightOuterColorOverride, colorValue);
#else
        half3 proximityColor = MixProximityLightColor(_ProximityLightData[dataIndex + 3], _ProximityLightData[dataIndex + 4], _ProximityLightData[dataIndex + 5], colorValue);
#endif  
    ...
    cornerCircleRadius = max(currentCornerRadius - _BorderWidth, _MinCorverValueWorld) * input.scale.z;
#else
    half borderMargin = _RoundCornerMargin + _BorderWidth * 0.5;
    cornerCircleRadius = saturate(max(currentCornerRadius - borderMargin, _MinCorverValue)) * input.scale.z;
#endif
    cornerCircleDistance = halfScale - (borderMargin * input.scale.z) - cornerCircleRadius;

    half borderValue = 1.0 - RoundCornersSmooth(cornerPosition, cornerCircleDistance, cornerCircleRadius, _EdgeSmoothingValue * input.scale.z);
    ...
    half3 borderColor = gradientColor;
#else
